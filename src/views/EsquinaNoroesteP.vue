<template>
  <div style="padding: 20px;">
    <h1 class="text-2xl font-bold mb-6">Método de la Esquina Noroeste</h1>
    ...
    <div class="mb-6">
      ...
      <textarea v-model="descripcionProblema" class="w-full p-2 border rounded" rows="4"
        placeholder="Describa el problema que desea resolver con esquina noroeste..."></textarea>
    </div>
...
export default {
  ...
  methods: {
    ...
    async handleSolve() {
      ...
      try {
        const response = await fetch(`${this.$apiBaseUrl}/esquinanor`, {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            nombresOrigenes: this.nombresOrigenes, // Enviar nombres de orígenes
            nombresDestinos: this.nombresDestinos, // Enviar nombres de destinos
            descripcionProblema: this.descripcionProblema, // Enviar descripción
          }),
        });
        ...
        this.results = {
          ...
          U: result.U,
          V: result.V,
          interpretation: result.interpretation || "No se generó interpretación.", // Interpretación de Gemini
          sensitivity_analysis: result.sensitivity_analysis || {}, // Análisis de sensibilidad (opcional)
        };
      } catch (error) {
